...
import { AuxiliaryData, hash_transaction, TransactionUnspentOutput } from '@emurgo/cardano-serialization-lib-browser';
import { EmurgoModule } from '@lib/emurgo';
import { transactionMinFee } from '@lib/dcspark/utils';
import BigNumber from 'bignumber.js';
import { amountIsValid } from '../utils';
import { requestAccountKeys } from '@lib/account';
...
export const signAndSubmit = async (
  ...
  try {
    ...
    const metadata: AuxiliaryData | undefined = unsignedTransaction.auxiliary_data();
    ...
    if (sign && password && password.length){
      ...
      for (const addr of usedAddresses) {
        ...
        const ntxHash = hash_transaction(nTxBody);
        for (const addr of usedAddresses) {
          const address = allAddresses.find(a => a.address === addr);
          const accountKeys = await requestAccountKeys(currentAccount.encryptedPrivateKey, password, address.chain, address.index);
